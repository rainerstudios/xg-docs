...
"Aikar's Flags" are command-line options (flags) for common
...
collector, enhancing the performance for Minecraft servers.

All Minecraft server lineups from BerryByte come equipped with Aikar's flags by
default.

<Info>Aikar's Flags allocates all available memory to the server (unused memory is wasted memory) which may make the server appear to be using 100% RAM at all times. This is not what's actually being used rather what's being allocated to it. Install [Spark](https://spark.lucko.me/download) and run `/spark health` for accurate memory usage.</Info>
