...
<Warning>Aikar's Flags allocates all available memory to the server (unused memory is wasted memory) which may make the server appear to be using 100% RAM at all times. This is not what's actually being used rather what's being allocated to it. Use (Spark)[</Warning>
