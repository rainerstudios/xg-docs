...
- `-XX:+ParallelRefProcEnabled`: Enables parallel reference processing which
  helps improves performance and limit lag spikes.
- `-XX:MaxGCPauseMillis`: Sets the maximum amount of time that the JVM can spend
  on garbage collection. This can help reduce lag spikes.
- `-XX:+UnlockExperimentalVMOptions`: Unlocks experimental JVM options.
- `-XX:+DisableExplicitGC`: Disables explicit garbage collection, which can help
  reduce lag spikes.
...
- `-XX:+PerfDisableSharedMem`: Disables shared memory, which can help reduce lag
  spikes.
- `-XX:MaxTenuringThreshold`: Sets the maximum tenuring threshold, which can
  help reduce lag spikes.
